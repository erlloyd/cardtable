...
const packsInputFolder = "./src/game-modules/star-wars-unlimited/external/sets";

const outputDirLoadFile = "./src/game-modules/star-wars-unlimited/generated";
...
const doWork = async () => {
  try {
    ...
    // For star-wars-unlimited we can just copy the raw data
    console.log("Copying card metadata");
    for (const pFilename of packs) {
      fs.appendFileSync(
        outputFile,
        `"star-wars-unlimited_${pFilename}",\n`,
        (e) => {
          if (!!e) console.log(e);
        }
      );
      ...
      fs.copyFileSync(
        ...
        `${publicFolder}/star-wars-unlimited_${pFilename}`
      );
    }
